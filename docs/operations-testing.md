...
| Provider terminal invariant | Anthropic SSE, Google, OpenAI/Mistral, and Bedrock tests cover terminal/malformed tails, truncated tool JSON, post-finish deltas, tail usage, and missing metadata; exactly one terminal event is published while Smithy framing remains validated. |
...
| Credentialed Provider services | Opt-in Failsafe tests cover OpenAI, Anthropic, Google AI Studio/Vertex, Mistral, Bedrock, and a local OpenAI-compatible endpoint; absent credentials produce explicit skips. The no-credential CI assertion is exactly seven skips and is not evidence of live-service compatibility. |
